...
test.describe("Auth - CreateToken", () => {
  test("returns valid token with correct credentials", async ({ request }) => {
  ...
  test("returns bad credentials with wrong password", async ({ request }) => {
    const response = await request.post("/auth", {
      data: {
        username: testData.auth.username,
...
test.describe("Booking - GetBookingIds", () => {
  test("returns all booking IDs", async ({ request }) => {
  ...
  test("filters booking IDs by name", async ({ request }) => {
    const response = await request.get("/booking", {
      params: {
  ...
  test("filters booking IDs by checkin date", async ({ request }) => {
    const response = await request.get("/booking", {
      params: {
        checkin: "2024-01-01",
      },
...
test.describe("Booking - GetBooking", () => {
  test("returns booking details for valid ID", async ({ request }) => {
  ...
  test("returns 404 for non-existent booking ID", async ({ request }) => {
    const response = await request.get("/booking/9999999");

    expect(response.status()).toBe(404);
  });

  test("returns booking as JSON with Accept header", async ({ request }) => {
    const listResponse = await request.get("/booking");
    const list = await listResponse.json();
    const firstId = list[0].bookingid;
...
test.describe("Booking - CreateBooking", () => {
  test("creates new booking and returns bookingid", async ({ request }) => {
  ...
  test("creates booking without additionalneeds", async ({ request }) => {
  ...
  test("persists created booking and allows retrieval", async ({ request }) => {
...
test.describe("Booking - UpdateBooking", () => {
  ...
  test("updates booking using cookie token", async ({ request }) => {
  ...
  test("updates booking using Basic auth", async ({ request }) => {
    const response = await request.put(`/booking/${bookingId}`, {
      headers: {
  ...
  test("returns 403 when updating without auth", async ({ request }) => {
    const response = await request.put(`/booking/${bookingId}`, {
      data: testData.updateBooking,
    });
...
test.describe("Booking - DeleteBooking", () => {
  ...
  test("deletes booking using cookie token", async ({ request }) => {
    const createResponse = await request.post("/booking", {
      data: testData.createBooking,
    });
    const createBody = await createResponse.json();
  ...
  test("deletes booking using Basic auth", async ({ request }) => {
    const createResponse = await request.post("/booking", {
      data: testData.createBooking,
    });
    const createBody = await createResponse.json();
  ...
  test("returns 403 when deleting without auth", async ({ request }) => {
    const createResponse = await request.post("/booking", {
      data: testData.createBooking,
    });
    const createBody = await createResponse.json();
